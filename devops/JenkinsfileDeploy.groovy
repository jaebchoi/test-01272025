            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "test-01272025 authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "test-01272025 failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "test-01272025 deploying"
                ...
                slackSend color: "good",
                        message: "test-01272025 deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "test-01272025 failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "test-01272025 shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "test-01272025 failed to shut down properly"
            throw err
        }
    }
}
